    ...
    def handle(self, *args, **options):
        ...
        for feed in feeds:
            ...
            for entry in feed_data.entries:
                ...
                if guid and Article.objects.filter(guid=guid).exists():
                    self.stdout.write(
                        f"RSS feed {feed.title} with guid {guid} already exists in the database"
                    )
                    continue
                ...
                if Article.objects.filter(url=link).exists():
                    self.stdout.write(
                        f"RSS feed {feed.title} with link {link} already exists in the database"
                    )
                    continue
            ...
            logger.info(
                f"Successfully fetched and parsed RSS feeds for {feed.title}"
            )  # opposite
